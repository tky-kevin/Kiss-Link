 ...
 *
 * <p>執行方式：Android Studio → 右鍵此檔案 → Run 'GroupCredentialTest'
 * 或在終端機：{@code ./gradlew test}
 */
public class GroupCredentialTest {
    ...
    public void constructor_storesAllFields() {
        GroupCredential cred = new GroupCredential(
                "DIRECT-KL-AB12", "SecurePass99", "192.168.49.1", 47890);
    ...
    public void ssid_startsWithDirectPrefix() {
        ...
        assertTrue(
                "SSID 必須以 'DIRECT-' 開頭（Wi-Fi Direct 規範要求）",
                cred.getSsid().startsWith("DIRECT-"));
    }
    ...
    public void passphrase_meetsWpa2MinimumLength() {
        ...
        GroupCredential cred = makeCred("DIRECT-KL-AB12");
        assertTrue(
                "Passphrase 長度必須 >= 8（WPA2 要求）",
                cred.getPassphrase().length() >= 8);
    }
    ...
    public void goIpAddress_matchesAndroidP2pConvention() {
        ...
        GroupCredential cred = makeCred("DIRECT-KL-AB12");
        assertEquals(
                "GO IP 應為 Android 保留的 192.168.49.1",
                "192.168.49.1", cred.getGoIpAddress());
    }
